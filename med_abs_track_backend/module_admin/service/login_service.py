...
class LoginService:
        ...
        if not PwdUtil.verify_password(login_user.password, user[0].password):
            ...
            if password_error_count > 5:
                ...
                raise LoginException(
                    data="",
                    message="10分钟内密码已输错超过5次，账号已锁定，请10分钟后再试",
                )
            logger.warning("密码错误")
    ...
    async def __check_login_ip(cls, request: Request):
        ...
        black_ip_value = await request.app.state.redis.get(
            f"{RedisInitKeyConfig.SYS_CONFIG.key}:sys.login.blackIPList"
        )
    ...
    async def __check_login_captcha(cls, request: Request, login_user: UserLogin):
        ...
        captcha_value = await request.app.state.redis.get(
            f"{RedisInitKeyConfig.CAPTCHA_CODES.key}:{login_user.uuid}"
        )
    ...
    async def create_access_token(
        ...
        encoded_jwt = jwt.encode(
            to_encode, JwtConfig.jwt_secret_key, algorithm=JwtConfig.jwt_algorithm
        )
        return encoded_jwt
    ...
    async def get_current_user(
        ...
        if token == redis_token:
            ...
            post_ids = ",".join(
                [str(row.post_id) for row in query_user.get("user_post_info")]
            )
            role_ids = ",".join(
                [str(row.role_id) for row in query_user.get("user_role_info")]
            )
            ...
            current_user = CurrentUserModel(
                ...
                user=UserInfoModel(
                    **CamelCaseUtil.transform_result(query_user.get("user_basic_info")),
                    postIds=post_ids,
                    roleIds=role_ids,
                    dept=CamelCaseUtil.transform_result(
                        query_user.get("user_dept_info")
                    ),
                    role=CamelCaseUtil.transform_result(
                        query_user.get("user_role_info")
                    ),
                ),
            )
            return current_user
    ...
    async def get_current_user_routers(cls, user_id: int, query_db: AsyncSession):
        ...
        return [
            router.model_dump(exclude_unset=True, by_alias=True)
    ...
    def __generate_menus(cls, pid: int, permission_list: List[SysMenu]):
        ...
        for permission in permission_list:
            if permission.parent_id == pid:
                ...
                menu_list_data = MenuTreeModel(
                    **CamelCaseUtil.transform_result(permission)
                )
                if children:
        ...
        for permission in permission_list:
            router = RouterModel(
                ...
                meta=MetaModel(
                    ...
                    link=(
                        permission.path if RouterUtil.is_http(permission.path) else None
                    ),
                ),
            )
            ...
            if c_menus and permission.menu_type == MenuConstant.TYPE_DIR:
                ...
                children = RouterModel(
                    ...
                    name=RouterUtil.get_route_name(
                        permission.route_name, permission.path
                    ),
                    meta=MetaModel(
                        title=permission.menu_name,
                        icon=permission.icon,
                        ...
                        link=(
                            permission.path
                            if RouterUtil.is_http(permission.path)
                            else None
                        ),
                    ),
                    query=permission.query,
                ...
                router.meta = MetaModel(
                    title=permission.menu_name, icon=permission.icon
                )
                router.path = "/"
                children_list: List[RouterModel] = []
    ...
    @classmethod
    async def get_sms_code_services(
        cls, request: Request, query_db: AsyncSession, user: ResetUserModel
    ):
        """
        获取短信验证码service
        ...
        redis_sms_result = await request.app.state.redis.get(
            f"{RedisInitKeyConfig.SMS_CODE.key}:{user.session_id}"
        )
        if redis_sms_result:
            return SmsCode(
                **dict(
        ...
        return SmsCode(
            **dict(is_success=False, sms_code="", session_id="", message="用户不存在")
        )
    ...
    async def forget_user_services(
        ...
        if forget_user.sms_code == redis_sms_result:
            ...
            forget_user.user_id = (
                await UserDao.get_user_by_name(query_db, forget_user.user_name)
            ).user_id
...
class RouterUtil:
    ...
    def get_component(cls, menu: MenuTreeModel):
        ...
        if menu.component and not cls.is_menu_frame(menu):
            ...
            component = MenuConstant.INNER_LINK
        elif (menu.component is None or menu.component == "") and cls.is_parent_view(
            menu
        ):
