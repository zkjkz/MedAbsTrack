...
class ConfigService:
    ...
    async def get_config_list_services(
        ...
        :return: 参数配置列表信息对象
        """
        config_list_result = await ConfigDao.get_config_list(
    ...
    async def init_cache_sys_config_services(cls, query_db: AsyncSession, redis):
        ...
        config_all = await ConfigDao.get_config_list(
            query_db, ConfigPageQueryModel(**dict()), is_page=False
        )
    ...
    @classmethod
    async def check_config_key_unique_services(
        cls, query_db: AsyncSession, page_object: ConfigModel
    ):
        """
        校验参数键名是否唯一service
        ...
        config = await ConfigDao.get_config_detail_by_info(
            query_db, ConfigModel(configKey=page_object.config_key)
        )
    ...
    async def add_config_services(
        ...
        if not await cls.check_config_key_unique_services(query_db, page_object):
            raise ServiceException(
                message=f"新增参数{page_object.config_name}失败，参数键名已存在"
            )
        else:
            try:
    ...
    async def edit_config_services(
        ...
        if config_info.config_id:
            if not await cls.check_config_key_unique_services(query_db, page_object):
                raise ServiceException(
                    message=f"修改参数{page_object.config_name}失败，参数键名已存在"
                )
            else:
                try:
        ...
        if page_object.config_ids:
            ...
            try:
                ...
                for config_id in config_id_list:
                    config_info = await cls.config_detail_services(
                        query_db, int(config_id)
                    )
                    if config_info.config_type == CommonConstant.YES:
                        raise ServiceException(
                            message=f"内置参数{config_info.config_key}不能删除"
                        )
                    else:
                        await ConfigDao.delete_config_dao(
                            query_db, ConfigModel(configId=int(config_id))
                        )
    ...
    @classmethod
    async def refresh_sys_config_services(
