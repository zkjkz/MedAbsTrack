...
class DictTypeModel(BaseModel):
    ...
    status: Optional[Literal["0", "1"]] = Field(
        default=None, description="状态（0正常 1停用）"
    )
...
class DictDataModel(BaseModel):
    ...
    css_class: Optional[str] = Field(
        default=None, description="样式属性（其他样式扩展）"
    )
    list_class: Optional[str] = Field(default=None, description="表格回显样式")
    is_default: Optional[Literal["Y", "N"]] = Field(
        default=None, description="是否默认（Y是 N否）"
    )
    status: Optional[Literal["0", "1"]] = Field(
        default=None, description="状态（0正常 1停用）"
    )
