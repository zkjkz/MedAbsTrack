...
class MenuModel(BaseModel):
    ...
    is_frame: Optional[Literal[0, 1]] = Field(
        default=None, description="是否为外链（0是 1否）"
    )
    is_cache: Optional[Literal[0, 1]] = Field(
        default=None, description="是否缓存（0缓存 1不缓存）"
    )
    menu_type: Optional[Literal["M", "C", "F"]] = Field(
        default=None, description="菜单类型（M目录 C菜单 F按钮）"
    )
    visible: Optional[Literal["0", "1"]] = Field(
        default=None, description="菜单状态（0显示 1隐藏）"
    )
    status: Optional[Literal["0", "1"]] = Field(
        default=None, description="菜单状态（0正常 1停用）"
    )
