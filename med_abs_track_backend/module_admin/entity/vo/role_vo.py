...
class RoleModel(BaseModel):
    ...
    menu_check_strictly: Optional[Union[int, bool]] = Field(
        default=None, description="菜单树选择项是否关联显示"
    )
    dept_check_strictly: Optional[Union[int, bool]] = Field(
        default=None, description="部门树选择项是否关联显示"
    )
    status: Optional[Literal["0", "1"]] = Field(
        default=None, description="角色状态（0正常 1停用）"
    )
