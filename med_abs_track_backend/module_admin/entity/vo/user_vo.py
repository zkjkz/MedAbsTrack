...
class UserModel(BaseModel):
    ...
    sex: Optional[Literal["0", "1", "2"]] = Field(
        default=None, description="用户性别（0男 1女 2未知）"
    )
    avatar: Optional[str] = Field(default=None, description="头像地址")
    password: Optional[str] = Field(default=None, description="密码")
    status: Optional[Literal["0", "1"]] = Field(
        default=None, description="帐号状态（0正常 1停用）"
    )
...
class UserInfoModel(UserModel):
    ...
    role: Optional[List[Union[RoleModel, None]]] = Field(
        default=[], description="角色信息"
    )
...
class UserDetailModel(BaseModel):
    ...
    data: Optional[Union[UserInfoModel, None]] = Field(
...
class UserRoleResponseModel(BaseModel):
    ...
    roles: List[Union[SelectedRoleModel, None]] = Field(
        default=[], description="角色信息"
    )
