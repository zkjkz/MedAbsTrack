...
class UserLogin(BaseModel):
    ...
    login_info: Optional[dict] = Field(
        default=None, description="登录信息，前端无需传递"
    )
    captcha_enabled: Optional[bool] = Field(
        default=None, description="是否启用验证码，前端无需传递"
    )
...
class MenuTreeModel(MenuModel):
    children: Optional[Union[List["MenuTreeModel"], None]] = Field(
...
class MetaModel(BaseModel):
    ...
    no_cache: Optional[bool] = Field(
        default=None, description="设置为true，则不会被 <keep-alive>缓存"
    )
