...
class OperLogModel(BaseModel):
    ...
    business_type: Optional[
        ...
        default=None,
        description="业务类型（0其它 1新增 2修改 3删除 4授权 5导出 6导入 7强退 8生成代码 9清空数据）",
    )
    ...
    status: Optional[Literal[0, 1, "0", "1"]] = Field(
        default=None, description="操作状态（0正常 1异常）"
    )
...
class LogininforModel(BaseModel):
    ...
    status: Optional[Literal["0", "1"]] = Field(
        default=None, description="登录状态（0成功 1失败）"
    )
