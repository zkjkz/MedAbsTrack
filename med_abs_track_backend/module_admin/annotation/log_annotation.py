...
class Log:
    ...
    def __call__(self, func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 处理文件路径，去除项目根路径部分
            relative_path = (
                os.path.relpath(file_path, start=project_root)[0:-2]
                .replace("\\", ".")
                .replace("/", ".")
            )
            # 获取当前被装饰函数所在路径
            ...
            request = get_function_parameters_value_by_name(
                func, request_name_list[0], *args, **kwargs
            )
            token = request.headers.get("Authorization")
            session_name_list = get_function_parameters_name_by_type(func, AsyncSession)
            query_db = get_function_parameters_value_by_name(
                func, session_name_list[0], *args, **kwargs
            )
            ...
            if self.log_type == "login":
                ...
                if request_from_swagger or request_from_redoc:
                    ...
                    await LoginLogService.add_login_log_services(
                        query_db, LogininforModel(**login_log)
                    )
            else:
                current_user = await LoginService.get_current_user(
                    request, token, query_db
                )
                oper_name = current_user.user.user_name
                dept_name = (
                    current_user.user.dept.dept_name if current_user.user.dept else None
                )
                ...
                await OperationLogService.add_operation_log_services(
                    query_db, operation_log
                )
