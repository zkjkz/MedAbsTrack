...
from module_admin.entity.vo.login_vo import SmsCode, UserLogin, UserRegister, Token
...
async def get_login_user_routers(
    ...
    logger.info("获取成功")
    user_routers = await LoginService.get_current_user_routers(
        current_user.user.user_id, query_db
    )
...
async def register_user(
    ...
    user_register_result = await LoginService.register_user_services(
        request, query_db, user_register
    )
    logger.info(user_register_result.message)

    return ResponseUtil.success(
        data=user_register_result, msg=user_register_result.message
    )


@loginController.post("/getSmsCode", response_model=SmsCode)
async def get_sms_code(
    request: Request, user: ResetUserModel, query_db: AsyncSession = Depends(get_db)
):
    try:
...
async def forget_user_pwd(
    ...
    try:
        forget_user_result = await LoginService.forget_user_services(
            request, query_db, forget_user
        )
        if forget_user_result.is_success:
            logger.info(forget_user_result.message)
            return ResponseUtil.success(
                data=forget_user_result, msg=forget_user_result.message
            )
        else:
