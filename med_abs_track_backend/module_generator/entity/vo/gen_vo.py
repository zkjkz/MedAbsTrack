...
class GenTableBaseModel(BaseModel):
    ...
    sub_table_fk_name: Optional[str] = Field(
        default=None, description="子表关联的外键名"
    )
    class_name: Optional[str] = Field(default=None, description="实体类名称")
    tpl_category: Optional[str] = Field(
        default=None, description="使用的模板（crud单表操作 tree树表操作）"
    )
    tpl_web_type: Optional[str] = Field(
        default=None, description="前端模板类型（element-ui模版 element-plus模版）"
    )
    ...
    gen_type: Optional[Literal["0", "1"]] = Field(
        default=None, description="生成代码方式（0zip压缩包 1自定义路径）"
    )
    gen_path: Optional[str] = Field(
        default=None, description="生成路径（不填默认项目路径）"
    )
...
class GenTableModel(GenTableBaseModel):
    ...
    pk_column: Optional["GenTableColumnModel"] = Field(
        default=None, description="主键信息"
    )
    sub_table: Optional["GenTableModel"] = Field(default=None, description="子表信息")
    columns: Optional[List["GenTableColumnModel"]] = Field(
        default=None, description="表列信息"
    )
    ...
    parent_menu_name: Optional[str] = Field(
        default=None, description="上级菜单名称字段"
    )
    ...
    def check_some_is(self) -> "GenTableModel":
        self.sub = (
            True
            if self.tpl_category and self.tpl_category == GenConstant.TPL_SUB
            else False
        )
        self.tree = (
            True
            if self.tpl_category and self.tpl_category == GenConstant.TPL_TREE
            else False
        )
        self.crud = (
            True
            if self.tpl_category and self.tpl_category == GenConstant.TPL_CRUD
            else False
        )
        return self
...
class EditGenTableModel(GenTableModel):
    ...
    params: Optional["GenTableParamsModel"] = Field(
        default=None, description="业务表参数"
    )
...
class GenTableColumnModel(GenTableColumnBaseModel):
    ...
    usable_column: Optional[bool] = Field(
        default=None, description="是否为基类字段白名单"
    )

    @model_validator(mode="after")
    def check_some_is(self) -> "GenTableModel":
        self.cap_python_field = (
            self.python_field[0].upper() + self.python_field[1:]
            if self.python_field
            else None
        )
        self.pk = True if self.is_pk and self.is_pk == "1" else False
        self.increment = (
            True if self.is_increment and self.is_increment == "1" else False
        )
        ...
        self.super_column = (
            True
            if StringUtil.equals_any_ignore_case(
                self.python_field, GenConstant.TREE_ENTITY + GenConstant.BASE_ENTITY
            )
            else False
        )
        self.usable_column = (
            True
            if StringUtil.equals_any_ignore_case(
                self.python_field, ["parentId", "orderNum", "remark"]
            )
            else False
        )
        return self
